...
import {
  registerAeliraDevApi,
  type AeliraWorldDevApi,
  type DevScreenshotOpts
} from "../dev/worldDevAccess.js";

type Props = {
  terrainSeed: number;
};

export function DevWorldCapture({ terrainSeed }: Props) {
  const { gl, scene } = useThree();
  const devCam = useMemo(() => new THREE.PerspectiveCamera(60, 1, 0.1, 600), []);

  useEffect(() => {
    const captureScreenshot = (opts: DevScreenshotOpts): string => {
    ...
    const api: AeliraWorldDevApi = { version: 1, captureScreenshot };
