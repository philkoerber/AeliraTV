...
function SunLights() {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("hemisphereLight", { args: [0xd7e3ff, 0x101622, 0.55] }), _jsx("primitive", { object: sun }), _jsx("primitive", { object: sun.target })] }));
}
function PointerLock({ yawRef }) {
    const { gl } = useThree();
    useEffect(() => {
...
function Centerpiece() {
    ...
    const dressMat = useMemo(() => new THREE.MeshStandardMaterial({ color: 0x6b4b7a, roughness: 1, flatShading: true }), []);
    const skinMat = useMemo(() => new THREE.MeshStandardMaterial({ color: 0xd6b89a, roughness: 1, flatShading: true }), []);
    const hairMat = useMemo(() => new THREE.MeshStandardMaterial({ color: 0x2a1f33, roughness: 1, flatShading: true }), []);
    const pianoMat = useMemo(() => new THREE.MeshStandardMaterial({ color: 0x1d1f28, roughness: 0.9, flatShading: true }), []);
    useEffect(() => {
    ...
    return _jsx("primitive", { object: group });
}
function WorldRoot({ room, displayName }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(PointerLock, { yawRef: yawRef }), _jsx(SunLights, {}), _jsx(TerrainChunks, {}), _jsx(Centerpiece, {}), playerIds.map((id) => {
